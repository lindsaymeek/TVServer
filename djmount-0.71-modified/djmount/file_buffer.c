...
void FileBuffer_SetSize(FileBuffer *file, off_t size)
{
	if(file) file->file_size=size;
}
...
*/
		...
		if (file->file_size > 0) {
			if (offset >= file->file_size - size) {
				size = MAX (0, file->file_size - offset -1);
				Log_Printf (LOG_DEBUG, 
		...
		if(file->file_size > 0)
		{
			...
			{
				printf("read: SEEK PAST END - IGNORED\n");
				return 0;
			}
		}


			rc = UpnpOpenHttpGetEx (file->url, &handle,
